...
extension DataRequest {
    ...
    public static func ObjectArraySerializer<T: Codable>(_ keyPath: String?) -> DataResponseSerializer<BaseResponse<[T]>> {
        return DataResponseSerializer { request, response, data, error in
            if let error = checkResponseForError(request: request, response: response, data: data, error: error){
                return .failure(error)
            }
            ...
            do {
                if let JSONObject = JSONObject, let newData = try? JSONSerialization.data(withJSONObject: JSONObject, options: [.prettyPrinted]){
                    let object = try JSONDecoder().decode(BaseResponse<[T]>.self, from: newData)
                    return .success(object)
                } else {
                    let failureReason = "JSONDecoder failed to serialize response."
                    let error = newError(.dataSerializationFailed, failureReason: failureReason)
    ...
    /// - Returns:              The request.
    @discardableResult
    public func responseArray<T: Codable>(queue: DispatchQueue? = nil, keyPath: String? = nil, completionHandler: @escaping (DataResponse<BaseResponse<[T]>>) -> Void) -> Self {
